...
object FrenchFerrumText : FerrumTextCatalog {
    ...
    override fun advice(key: FerrumAdviceKey, arguments: Map<String, Any?>): String {
        val template = ADVICE.getValue(key)
        val expected = PLACEHOLDER.findAll(template).map { it.groupValues[1] }.toSet()
        require(arguments.keys == expected) { "Arguments for $key must be $expected but were ${arguments.keys}" }
        return PLACEHOLDER.replace(template) { arguments.getValue(it.groupValues[1]).toString() }
    }
